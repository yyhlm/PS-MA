function summary = summarize_allm_campaign(results, config)

n = numel(results);
steps = 0:config.recordEvery:config.T;
summary.numReplicates = n;
summary.recordSteps = steps;
for m = 1:numel(config.methods)
    key = config.methods{m};
    finalRegret = zeros(n, 1);
    finalDelay = zeros(n, 1);
    runtime = zeros(n, 1);
    switchCount = zeros(n, 1);
    regret = zeros(n, numel(steps));
    delay = zeros(n, numel(steps));
    for r = 1:n
        result = results{r}.(key);
        finalRegret(r) = result.finalRegret;
        finalDelay(r) = result.finalDelay;
        runtime(r) = result.runtimeSeconds;
        switchCount(r) = sum(diff(result.selectedArm) ~= 0);
        regret(r, :) = result.Regret;
        delay(r, :) = result.Delay;
    end
...
if any(strcmp(config.methods, 'psma'))
    comparisons = struct();
    psma = cellfun(@(r) r.psma.finalRegret, results)';
    for m = 1:numel(config.methods)
        key = config.methods{m};
        if strcmp(key, 'psma')
            continue;
        end
        delta = cellfun(@(r) r.(key).finalRegret, results)' - psma;
...
statistics.mean = mean(values, 1);
statistics.std = std(values, 0, 1);
