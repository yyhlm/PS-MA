function paths = export_tcp_comparison_table(comparison)
...
regretMean = zeros(numel(methods), 1);
regretStd = regretMean; regretLow = regretMean; regretHigh = regretMean;
delayMean = regretMean; delayStd = regretMean;
runtimeMean = regretMean; switchMean = regretMean;
for i = 1:numel(methods)
    methodKey{i} = methods{i};
    methodLabel{i} = tcp_method_label(methods{i});
    source = comparison.summary.(methods{i});
...
methodTable = table(methodKey, methodLabel, regretMean, regretStd, regretLow, regretHigh, ...
    delayMean, delayStd, runtimeMean, switchMean);
paths.method = fullfile(outputDirectory, [comparison.runKey, '_methods.csv']);
writetable(methodTable, paths.method);

comparisons = comparison.summary.pairedEnvironmentComparisons;
...
paths.paired = fullfile(outputDirectory, [comparison.runKey, '_paired_environment.csv']);
